...
const profiles = ['acp', 'sdk', 'session', 'tui', 'web'] as const
...
async function scenarios(): Promise<Scenario[]> {
  ...
  for (const profile of profiles) {
    ...
    for (const entry of await readdir(root, { withFileTypes: true })) {
      ...
      const expectedProfile = profile === 'session' ? 'headless'
        : profile === 'tui' ? 'deepseek-tui' : profile
      expect(manifest.profile, `${profile}/${entry.name}: profile`).toBe(expectedProfile)
...
it('keeps every recorded session owned, pinned, redacted, and header-scrubbed', async () => {
  ...
  for (const scenario of all) {
    ...
    expect(existsSync(join(dir, 'terminal.expected.txt')), `${key}: TUI evidence ownership`)
      .toBe(scenario.profile === 'tui')
