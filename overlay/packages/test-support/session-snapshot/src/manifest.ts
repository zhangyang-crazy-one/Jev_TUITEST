...
export type SnapshotProfile = 'headless' | 'sdk' | 'acp' | 'web' | 'deepseek-tui'
...
const PROFILES = new Set<SnapshotProfile>(['headless', 'sdk', 'acp', 'web', 'deepseek-tui'])
...
export function parseSnapshotManifest(source: string, path = 'snapshot.yml'): SnapshotManifest {
  ...
  try {
    ...
    if (typeof root.profile !== 'string' || !PROFILES.has(root.profile as SnapshotProfile)) {
      throw new Error('manifest.profile must be headless, sdk, acp, web, or deepseek-tui')
    }
