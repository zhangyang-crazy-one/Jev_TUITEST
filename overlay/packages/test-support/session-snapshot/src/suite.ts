/**
 * Keyless recorded-session suite factory. Each scenario drives its controlled
 * interface and compares deterministic output; comparable session fixtures are
...
import { type AgentUnderTest, type HarvestedLog, type InputScript, type RunOptions, type RunResult, runScenario } from './harness.ts'
import {
...
/** Interface-specific input and output around the shared Session comparisons. */
export interface SnapshotSuiteAdapter {
  /** File holding deterministic interface evidence. */
  outputFile: string
  /**
   * Run the interface to quiescent process exit before returning harvested logs.
   * @param options - Selected replay files and isolated launch settings.
   * @param scenario - Scenario metadata for the interface controller.
   * @returns Interface evidence, workspace state, and persisted logs.
   */
  run: (options: RunOptions, scenario: Scenario) => Promise<RunResult>
}

/**
 * Register shared Session comparisons and guards with an optional interface adapter.
 * @param options - Profile paths, scenario table, and snapshot mode.
 * @param adapter - Interface runner; absent selects ACP input and stdout.
 */
export function defineSessionSnapshotSuite(options: SnapshotSuiteOptions, adapter?: SnapshotSuiteAdapter): void {
  ...
  scenarioSuite('snapshot scenarios', () => {
    for (const scenario of scenarios) {
      ...
      it.skipIf(scenarioSkipped(scenario, RECORDING, process.platform, options.hasPwsh))(`snapshot: ${scenario.name} matches the expected outputs`, async ({ expect }) => {
        ...
        // ACP record input lives in its control script; other adapters derive
        // their task from an existing selected Session in every mode.
        let fixtureFiles = RECORDING && adapter === undefined ? [] : await sessionFixtures(dir)
        ...
        const result = adapter === undefined
          ? await runScenario(JSON.parse(await readFile(join(dir, 'input.json'), 'utf8')) as InputScript, runOptions)
          : await adapter.run(runOptions, scenario)
        ...
        const outputVariants = adapter === undefined
          ? stdoutExpectedVariants(scenario)
          : [{ file: adapter.outputFile, cwdPathMode: 'canonical' as const }]
        for (const expected of outputVariants) {
          const stdout = adapter === undefined
            ? normalizeStdout(result.rawStdout, ctx, { cwdPathMode: expected.cwdPathMode })
            : result.rawStdout
          if (REFRESHING) {
  ...
  describe('snapshot fixtures', () => {
    ...
    it('every registered scenario has its required fixture files', async () => {
      ...
      for (const { name, overridden, pinsNativeWindowsStdout, pinsChildToolSchemas, pinsChildSystemPrompts } of scenarios) {
        ...
        expect(manifest.session, `${name}: scenarios own their session`).toBeUndefined()
        ...
        expect(existsSync(join(dir, 'input.json')), `${name}/input.json presence`).toBe(adapter === undefined)
        const outputFile = adapter?.outputFile ?? 'stdout.expected.jsonl'
        expect(existsSync(join(dir, outputFile)), `${name}/${outputFile}`).toBe(true)
        expect(
